async function dapp() {
    ...
    if (typeof web3=='undefined') {
        ...
        contract = new web3.eth.Contract(contractData.abi, contractData.networks[networkId].address);
        ...
        if(userAccount !== owner){
            $('#content').html("Please connect the dapp with the owners accounts ("+owner+").");
            ...
            $('#description').html("Data Feed: <input id='newDesc' type='text' value='"+description+"'></input><button id='upDesc'>Update Description</button>");
            $('#price').html("Price: <input id='newPrice' type='text' value='"+price.toString()+"'></input><button id='upPrice'>Update Price</button>");
            $('#url').html("Url: <input id='newUrl' type='text' value='"+url+"'></input><button id='upUrl'>Update Url</button>");
            createButtonHandler();
        }     
    }
    ...
    function createButtonHandler(){
        $("#upDesc").click(function(){
            contract.methods.setDescription($("#newDesc").val()).send({from:userAccount});
        });
        $("#upPrice").click(function(){
            contract.methods.setPrice($("#newPrice").val()).send({from:userAccount});
        });
