  ...
  if (req.query.l == 'true') {
    ...
    if (sess.admin == true) {
      ...
      if (req.query.s !== undefined) {
        ...
        prefixedMsg = eutils.keccak(Buffer.concat([eutils.toBuffer("\x19Ethereum Signed Message:\n32"), eutils.toBuffer(shash)]));
        signer = eutils.ecrecover(prefixedMsg,sigvals.v,sigvals.r,sigvals.s);
        signer = eutils.toChecksumAddress(eutils.bufferToHex(eutils.pubToAddress(signer)));
   ...
   // if the admin is logged in load the open channels
    ...
    res.render('users', {shash:shash, loggedin:loggedin, openChannels:openChannels, closedChannels:closedChannels});
  }
});
