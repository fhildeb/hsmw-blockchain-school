...
import {
  ...
  const yourAccount = Account.createFromPrivateKey(
    yourPrivateKey,
    NetworkType.MIJIN_TEST
  );
  
  const ticketBuyersPublicKey = "ticket-buyers-public-key";
  const ticketBuyersPublicAccount = PublicAccount.createFromPublicKey(
    ticketBuyersPublicKey,
    NetworkType.MIJIN_TEST
  );
  ...
  listener.open().then(() => {
    ...
    listener
      ...
      .pipe(
        filter(
          (transaction: { transactionInfo: { hash: any; } | undefined; }) =>
            transaction.transactionInfo !== undefined &&
            transaction.transactionInfo.hash === lockFundsTransactionSigned.hash
        ),
        mergeMap((ignored: any) =>
          transactionHttp.announceAggregateBonded(signedTransaction)
        )
      )
      .subscribe(
        (announcedAggregateBonded: any) => console.log(announcedAggregateBonded),
        function (err: any) {
